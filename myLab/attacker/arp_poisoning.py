...
def enable_ip_forwarding():
    ...
    try:
        ...
        print("If you are in a restricted container (like Kathara), ensure it is enabled in the lab configuration.")
        print("If you are on a real machine, ensure you are running as root.")
...
def prime_arp_cache(target_ip, gateway_ip):
    ...
    """
    print(f"Priming ARP cache for {target_ip}...")
...
def run_arp_spoof(target_ip, gateway_ip, dns_ip=None):
    try:
        ...
        print(f"Gateway/Spoof IP: {gateway_ip}")
        if dns_ip:
            print(f"Also spoofing DNS server mapping on target: {dns_ip}")
        ...
        while True:
            ...
            print(f"\rSUCCESS: Packets sent: {sent_packets_count}", end="")
            time.sleep(2)
...
def cleanup_and_exit(signum, frame):
    # Best-effort cleanup: restore poisoned ARP entries
    try:
        print('\n!!! Cleanup: Restoring ARP tables...')
